...
class CustomStableDiffusionControlNetPipeline(StableDiffusionControlNetPipeline):
    ...
    def __call__(
        ...
        # 5. Prepare timesteps
        self.scheduler.set_timesteps(num_inference_steps, device=device)
        if pregen_images[0] is None or (isinstance(prompt, list) and len(prompt) != len(pregen_images)):
            timesteps = self.scheduler.timesteps
        else:
            timesteps, num_inference_steps = self.get_timesteps(
                num_inference_steps, strength, device
        ...
        if pregen_images[0] is None or (isinstance(prompt, list) and len(prompt) != len(pregen_images)):
            ...
            print("pregen")
            pregen_image = torch.cat([self.preprocess_image(pregen_images[i]) for i in range(len(prompt))], dim=0)
            latents, _, _ = self.prepare_latents_pregen(
                pregen_image,
                1,
                num_images_per_prompt,
                prompt_embeds.dtype,
            ...
            # print(latents.shape)
            # latents = latents[:1].repeat(len(pregen_image), 1, 1, 1)
